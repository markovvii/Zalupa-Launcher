...
class Settings {
    ...
    async initRam() {
        ...
        document.getElementById("total-ram").textContent = `${totalMem} ГБ`;
        document.getElementById("free-ram").textContent = `${freeMem} ГБ`;
        ...
        minSpan.setAttribute("value", `${ram.ramMin} ГБ`);
        maxSpan.setAttribute("value", `${ram.ramMax} ГБ`);

        slider.on("change", (min, max) => {
            minSpan.setAttribute("value", `${min} ГБ`);
            maxSpan.setAttribute("value", `${max} ГБ`);
    ...
    async initJavaPath() {
        ...
        let javaPath = javaDatabase ? javaDatabase : 'Используйте версию Java, поставляемую с лаунчером';
        ...
        document.querySelector(".path-button").addEventListener("click", async() => {
            ...
            } else alert("Имя файла должно быть java или javaw.");

        });

        document.querySelector(".path-button-reset").addEventListener("click", () => {
            path.value = 'Используйте версию Java, поставляемую с лаунчером';
            file.value = '';
