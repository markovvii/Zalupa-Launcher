    ...
    async initNews() {
        ...
        if (this.news) {
                ...
                blockNews.innerHTML = `
                    <div class="news-header">
                        <div class="header-text">
                            <div class="title">В настоящее время новостей нет.</div>
                        </div>
                    </div>
                    <div class="news-content">
                        <div class="bbWrapper">
                            <p>Здесь вы можете следить за всеми новостями о сервере.</p>
                        </div>
                    </div>`
            ...
            blockNews.innerHTML = `
                ...
                <div class="news-content">
                    <div class="bbWrapper">
                        <p>Невозможно связаться с сервером новостей.</br>Пожалуйста, проверьте вашу конфигурацию.</p>
                    </div>
                </div>`
            // news.appendChild(blockNews);
        }
    ...
    async initLaunch() {
        document.querySelector('.play-btn').addEventListener('click', async () => {
            ...
            launch.on('progress', (progress, size) => {
                ...
                document.querySelector(".text-download").innerHTML = `Загрузка ${((progress / size) * 100).toFixed(0)}%`
            ...
            launch.on('check', (progress, size) => {
                progressBar.style.display = "block"
                document.querySelector(".text-download").innerHTML = `Верификация ${((progress / size) * 100).toFixed(0)}%`
                progressBar.value = progress;
                progressBar.max = size;
            });
